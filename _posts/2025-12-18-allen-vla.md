...
graph TB
    subgraph Hardware["🔧 Hardware Components"]
        subgraph RobotArms["Robot Arms"]
            LEADER["👆 SO101 Leader Arm<br/>Manual Control"]
            FOLLOWER["🤖 SO101 Follower Arm<br/>5-DOF + Gripper"]
        end
        CAM["📷 USB Camera<br/>640×480 @ 30Hz"]
        SERIAL["🔌 Serial Interface<br/>/dev/ttyACM0-1"]
    end
...
```mermaid
graph LR
    subgraph Joints["🤖 SO101 Joint Configuration"]
        J1["1️⃣ shoulder_pan<br/>Base rotation"]
        J2["2️⃣ shoulder_lift<br/>Shoulder elevation"]
        J3["3️⃣ elbow_flex<br/>Elbow bend"]
        J4["4️⃣ wrist_flex<br/>Wrist pitch"]
        J5["5️⃣ wrist_roll<br/>Wrist rotation"]
        J6["6️⃣ gripper<br/>Open/Close"]
    end
...
graph LR
    subgraph Encoder["🔍 Encoder"]
        IMG["📷 Image<br/>640×480×3"]
        STATE["🤖 Joint State<br/>6 DOF"]
    ...
    subgraph ActionChunk["🎯 Action Chunk"]
        DECODER --> FC["Fully Connected"]
        FC --> CHUNK["Action Chunk<br/>T × 6 DOF"]
    end
...
graph TB
    ...
    subgraph Hardware["🔧 Hardware Layer"]
        subgraph Arms["Robot Arms"]
            LEADER["👆 Leader Arm<br/>Manual Control"]
            FOLLOWER["🤖 Follower Arm<br/>5-DOF + Gripper"]
        end
        CAM["📷 Camera<br/>640×480"]
        SERIAL["🔌 Serial<br/>/dev/ttyACM*"]
    end
    ...
    subgraph Cloud["☁️ Cloud Services"]
        HF_DATA["🤗 Hub<br/>Datasets"]
        HF_MODEL["🤗 Hub<br/>Models"]
        WANDB["📈 W&B<br/>Metrics"]
    end
...
```mermaid
graph TB
    subgraph LeRobotCore["🤗 LeRobot Core"]
    ...
    subgraph SO101Integration["🔧 SO101 Integration"]
    ...
    subgraph Features["📊 Feature Configuration"]
        ...
        CAMERA["Camera Feature<br/>H×W×C Images"]
...
sequenceDiagram
    ...
    participant 👆 as Leader Arm
    participant 💻 as Control System
    participant 🤖 as Follower Arm
    participant 📷 as Camera Thread
    participant 🤗 as HuggingFace Hub

    👤->>👆: Manipulate leader arm
    ...
    alt Press ENTER → Recording ON
        💻->>💻: Start new episode (UUID)
        loop Every timestep
            💻->>💻: Record observation + action
        end
    end

    alt Press ENTER → Recording OFF
        💻->>💻: Save episode to LeRobotDataset
    end

    alt Press CTRL+C → Finalize
        💻->>💻: Consolidate all episodes
        💻->>🤗: Push dataset to Hub
        Note over 🤗: username/dataset-name
    end
```
...
graph LR
    subgraph Input["📥 Input Streams"]
        CAM_THREAD["📷 Camera Thread<br/>Non-blocking capture"]
        LEADER["👆 Leader Arm<br/>Joint positions"]
    end
...
```mermaid
sequenceDiagram
    participant 🤖 as Robot (Jetson)
    participant 📡 as TCP Client
    participant 🌐 as Network
    participant 🖥️ as TCP Server
    participant 🎮 as GPU (CUDA)

    🤖->>📡: Camera RGB + Joint state
    📡->>🌐: pickle(observation)
    Note over 🌐: 4-byte length prefix<br/>+ serialized data
    🌐->>🖥️: Forward request
    🖥️->>🎮: Policy forward pass
    🎮->>🖥️: Action chunk [T×6]
    🖥️->>🌐: pickle(actions)
    🌐->>📡: Forward response
    📡->>🤖: Execute actions

    Note over 🤖,🎮: Episode boundary
    📡->>🖥️: Reset signal
    🖥️->>🖥️: Clear policy state
```
...
flowchart TD
    subgraph DataPhase["📊 Data Phase"]
        START([Start]) --> COLLECT["👤 Teleoperation<br/>Collect Demos"]
        COLLECT --> UPLOAD["🤗 Push to Hub"]
    end
...
3. **Training**: GPU-accelerated with Accelerate, logged to W&B
4. **Checkpointing**: Save every 5K steps, push best to 🤗 Hub
5. **Deployment**: Local or distributed inference architecture
6. **Evaluation**: Real-world testing, iterate on failures
...
graph LR
    subgraph Phase1["✅ Phase 1 - Complete"]
        ...
        P1C["🤗 Hub Integration<br/>Auto upload"]
        P1D["Calibration<br/>Joint zeros"]
    end

    subgraph Phase2["⚙️ Phase 2 - In Progress"]
        P2A["✅ ACT Training<br/>Fully implemented"]
        P2B["✅ ACT Inference<br/>Local + Distributed"]
        P2C["✅ Jetson Deploy<br/>Scripts ready"]
        P2D["⏳ π0/π0.5<br/>Planned"]
        P2E["⏳ Benchmarks<br/>In progress"]
    end
...
- ⏳ Working on additional VLA models (π0, π0.5)
- ⏳ Real-world performance evaluation in progress
...
| Training | `policy/act/train.py` | ACT training with W&B |
| Local Inference | `policy/act/inference.py` | Same-machine execution |
| Server Inference | `policy/act/inference_server.py` | GPU server for distributed |
| Client Inference | `policy/act/inference_client.py` | Robot client for distributed |
...
graph TB
    ...
    subgraph Robot["🤖 Robot Learning"]
        LEROBOT["🤗 LeRobot v0.4.2+"]
...
The integration of 🤗 LeRobot with SO101 provides an accessible platform for robot learning research, and the insights gained from training and evaluating multiple VLA models will contribute to understanding the capabilities and limitations of these emerging approaches.
...
3. **π0**: A Vision-Language-Action Flow Model for General Robot Control - [Physical Intelligence](https://www.physicalintelligence.company/blog/pi0)
