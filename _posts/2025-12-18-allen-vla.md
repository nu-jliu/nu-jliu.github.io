...
graph TB
    subgraph Hardware["Hardware Components"]
        ...
        CAM["USB Camera<br/>640x480 @ 30Hz"]
    end

    subgraph Software["Software Stack"]
        LEROBOT["LeRobot Framework"]
        TELEOP["Teleoperation<br/>20-30 Hz"]
...
graph LR
    ...
    subgraph GPU["GPU Server"]
        SERVER["TCP Server<br/>Port 8000"]
        POLICY["VLA Policy<br/>ACT/Diffusion"]
    end

    CAM --> CLIENT
    ARM --> CLIENT
    CLIENT <-->|Observations/Actions| SERVER
    SERVER --> POLICY
    CLIENT --> ARM
